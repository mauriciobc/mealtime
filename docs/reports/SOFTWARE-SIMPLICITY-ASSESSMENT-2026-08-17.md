...
## Addendum — after one-owner work (2026-08-17)

The scorecard above is the measurement at `151b17f`. **Formulas and baseline scores are not restated.** This addendum only re-runs the same five scenarios against the code after P0–P2 consolidation (authz helper, one cat schema/hook, v2-client mappers, dead-layer delete, Zod on remaining mutations, v1 stubs → one catch-all).

### Same five scenarios (production files to change the rule)

| Scenario | Before | After | Why |
| --- | ---: | ---: | --- |
| 1. Record a feeding | 10 | ~8 | Mapping lives in `lib/mappers/feeding.ts`; the domain hook only calls `v2Get`. POST `/api/v2/feedings` and FeedingContext were not split (out of scope). |
| 2. Invite / accept a household member | 8 | ~5 | Admin check is `requireHouseholdAdmin`. Invite body already had Zod; invite-code / accept / reject now validate ids with `uuidParamSchema`. |
| 3. Add a field on cat | 20 | **3** | One Zod owner: `lib/validations/cats.ts`, imported by the three live cat write routes. |
| 4. Log a weight | 6 | ~4 | One DateTimePicker (`components/ui/datetime-picker.tsx`). WeightPageContent was not split (out of scope). |
| 5. Tighten household authorization | ~19 | **1–2** | Policy lives in `lib/authz/household-access.ts`. A new route only calls the helper. |

**Change amplification after:** values `8, 5, 3, 4, 2`. **Median 4** (was 10). **Range 2–8** (was 6–20).

### Validation criteria from the original findings

| Criterion | After |
| --- | --- |
| Membership `find*` for **authorization** in `app/api/v2` | Helper module + justified exceptions (membership mutations, deliver cron) |
| `user.household_ids` | All memberships; stats/cats no longer filter on the first id only |
| One cat Zod + one `useCats` cache | `lib/validations/cats.ts`; `domainKeys.cats`; CatsContext has no reducer |
| Domain hooks | No raw `fetch` in `lib/hooks/domain` |
| DateTimePicker import paths in app code | `datetime-picker` only |
| Architecture docs | Next.js 16, Supabase (not NextAuth), React Query for domain data; `docs/architecture/authz.md` |
| Knip | No ignore for `lib/dto/**`, `lib/validations/**`, `feeding-service`, dead repositories |
| v1 stubs | Deleted; `proxy.ts` + `app/api/[...v1]/route.ts` still return 410. Smoke: 410 `/api/feedings`, 401 `/api/v2/cats` |

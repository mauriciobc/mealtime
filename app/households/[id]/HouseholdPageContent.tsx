...
import { v2Delete, v2Patch } from "@/lib/api/v2-client"
...
export default function HouseholdPageContent({ params }: HouseholdPageContentProps) {
  ...
  const loadError = householdQueryError ? (householdQueryError instanceof Error ? householdQueryError.message : "Failed to load household") : null;
  ...
  const leaveHousehold = async () => {
    ...
    try {
      await v2Delete(`/api/v2/households/${household.id}/members/${currentUser.id}?leave=true`);
  ...
  useEffect(() => {
    ...
    const isMember = householdData.members?.some((m: { userId?: string }) => String(m.userId) === String(currentUser.id));
    if (!isOwner && !isMember) {
      setHousehold(null);
      setCats([]);
      return;
    }
    const householdCats = allCats.filter(cat => String(cat.householdId) === String(householdData.id));
  ...
  useEffect(() => {
    ...
    if (loadError.toLowerCase().includes("access") || loadError.toLowerCase().includes("permission")) {
      router.push("/households");
    }
  }, [loadError, router]);
